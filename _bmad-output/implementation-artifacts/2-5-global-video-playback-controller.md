...
6. **Интеграция в `GalleryGrid`** (Story 2.4): клик по видео-элементу в галерее открывает `VideoPlayer` (или переключает активное видео).

7. **Интеграция в `PostDetail`** (Story 2.3): видео-пост отображается через `VideoPlayer` вместо статичного `LazyMediaWrapper` с иконкой play.
...
- [ ] **Task 6: Интегрировать `VideoPlayer` в `PostDetail`** (AC: #7)
  - [ ] 6.1. В `src/components/feed/PostDetail.tsx`: заменить `LazyMediaWrapper` (type="video") на `VideoPlayer`
  - [ ] 6.2. Передать `src={post.imageUrl}` как `poster` (или `src` из `post_media` если Story 2.1 завершена)
  - [ ] 6.3. Передать уникальный `videoId={post.id}` (достаточно уникален для детальной страницы)
  - [ ] 6.4. Обновить тесты `PostDetail.test.tsx`

- [ ] **Task 7: Интегрировать `VideoPlayer` в `GalleryGrid`** (AC: #6)
  - [ ] 7.1. В `src/components/feed/GalleryGrid.tsx` (Task 2.4): для элементов с `media_type='video'` — рендерить `VideoPlayer` вместо статичного `LazyMediaWrapper`
  - [ ] 7.2. Использовать `videoId={media.id}` из `PostMedia.id` как уникальный идентификатор

- [ ] **Task 8: Верификация** (AC: #10)
  - [ ] 8.1. `npx tsc --noEmit` → 0 ошибок
  - [ ] 8.2. Запустить все тесты → регрессий нет
...
src/components/feed/PostDetail.tsx    # ← VideoPlayer вместо LazyMediaWrapper
