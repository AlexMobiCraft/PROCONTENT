...
-   [ ] [AI-Review][High][Logic] Исправить рендеринг одиночного видео в `PostCard.tsx`: условие рендеринга `VideoPlayerContainer` должно учитывать `post.type === 'multi-video'` (для фоллбэка одиночного элемента) и корректно передавать тип в `LazyMediaWrapper` `[src/components/feed/PostCard.tsx:110,128]`
-   [ ] [AI-Review][Medium][UX] GalleryGrid.tsx: убрать интерактивные стили (hover, focus-ring) с неинтерактивных оберток `<div>` вокруг видео `[src/components/feed/GalleryGrid.tsx:115-117,183-185]`
-   [ ] [AI-Review][Low][HTML] VideoPlayer.tsx: Исправить WebVTT data URI — добавить символ переноса строки (`%0A`) после `WEBVTT` для соответствия спецификации и устранения ворнингов в консоли `[src/components/media/VideoPlayer.tsx:124]`
...
- `src/components/media/VideoPlayer.tsx` — Dumb UI рефакторинг: убран useVideoController, добавлены onPlay/onPause/ref/isLoading пропсы, IntersectionObserver для viewport-autopause, skeleton; Memory Leak fix (hasError в deps), onPause при onError, eslint-disable captions
...
- `tests/unit/components/media/VideoPlayer.test.tsx` — добавлены тесты: IntersectionObserver disconnect on error, onPause on error
...
- `_bmad-output/implementation-artifacts/stories/2-5-global-video-playback-controller.md` — статус review, File List, completion notes (Round 7)
