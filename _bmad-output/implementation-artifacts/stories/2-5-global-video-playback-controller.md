...
- `src/components/media/LazyMediaWrapper.tsx` — добавлен `id?: string` в `MediaItem` для передачи videoId из mediaItem
- `src/components/media/VideoPlayer.tsx` — Dumb UI рефакторинг: убран useVideoController, добавлены onPlay/onPause/ref/isLoading пропсы, IntersectionObserver для viewport-autopause, skeleton; Memory Leak fix (hasError в deps); videoRef добавлен в deps (Round 14); убран title={alt} (оставлен aria-label, Round 14)
...
- `tests/unit/components/feed/PostCard.test.tsx` — мок обновлён на VideoPlayerContainer; добавлены тесты на mixed gallery navigation и `disabled` для Like; обновлён тест fallback videoId (`fallback-video-${post.id}`); добавлен тест mediaItem.id priority (Round 14)
