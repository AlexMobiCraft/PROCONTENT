...
- [ ] [AI-Review][Critical] Реализовать skeleton-placeholder в `VideoPlayer` по Task 3.7: добавить loading-state и рендеринг скелетона по паттерну `LazyMediaWrapper` `[src/components/media/VideoPlayer.tsx]`
- [ ] [AI-Review][High] Сделать `VideoPlayer` Dumb UI по AC #3: убрать `useVideoController` из компонента, добавить пропсы `onPlay` / `onPause` для callbacks извне `[src/components/media/VideoPlayer.tsx:6-19,30]`
- [ ] [AI-Review][High] Добавить тест на автопаузу при выходе из viewport в `VideoPlayer.test.tsx`: проверить `isInView=false → pause()` сценарий по AC #8 `[tests/unit/components/media/VideoPlayer.test.tsx]`
- [ ] [AI-Review][Medium] Синхронизировать `File List` с git reality: убрать отсутствующие в diff файлы, добавить story-файл в список изменений `[_bmad-output/implementation-artifacts/stories/2-5-global-video-playback-controller.md:499-513]`
- [ ] [AI-Review][Medium] Исправить A11y regression: убрать `aria-hidden="true"` с `Link` вокруг media-content, чтобы сохранить доступность превью для screen readers `[src/components/feed/PostCard.tsx:106-107,122-130]`
- [ ] [AI-Review][Low] Убрать дублирование `VideoPlayer` в `File List`: оставить только в «Новые файлы» `[_bmad-output/implementation-artifacts/stories/2-5-global-video-playback-controller.md:501,509]`
...
- `src/components/feed/GalleryGrid.tsx` — интеграция `VideoPlayer` для `media_type='video'`; `priority={priority && i < 2}` для видео в main grid
- `src/components/feed/PostCard.tsx` — импорт `VideoPlayer`; одиночное видео рендерит `VideoPlayer`; галерея с видео без `<Link>`
- `src/components/media/VideoPlayer.tsx` — `onClick={(e) => e.preventDefault()}` на wrapper div
- `tests/unit/components/feed/GalleryGrid.test.tsx` — обновлён mock VideoPlayer (data-priority); тесты проброса priority
- `tests/unit/components/feed/PostCard.test.tsx` — добавлен mock VideoPlayer; тесты одиночного видео и галереи с видео
- `tests/unit/components/media/VideoPlayer.test.tsx` — тест на preventDefault
