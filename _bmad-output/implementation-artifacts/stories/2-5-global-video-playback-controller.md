...
- `src/hooks/useVideoController.ts` — убран IntersectionObserver (перенесён в VideoPlayer); store-логика сохранена (isActive boolean-селектор, cleanup при unmount)
- `src/features/feed/components/VideoPlayerContainer.tsx` — добавлен `key={props.src}` для сброса hasError при смене src через ремаунт; State Leak fix: `useEffect` для сброса `activeVideoId` при смене src
...
- `tests/unit/components/feed/PostCard.test.tsx` — мок обновлён на VideoPlayerContainer; добавлены тесты на mixed gallery navigation и `disabled` для Like; обновлён тест fallback videoId (`fallback-video-${post.id}`)
- `tests/unit/components/media/VideoPlayer.test.tsx` — добавлены тесты: IntersectionObserver disconnect on error, onPause on error, переход `isLoading=true -> false`; обновлены тесты IntersectionObserver (intersectionRatio: 0.1); обновлён тест сброса hasError (wrapper с key={src})
...
- `_bmad-output/implementation-artifacts/stories/2-5-global-video-playback-controller.md` — статус review, File List, completion notes (Round 12)
