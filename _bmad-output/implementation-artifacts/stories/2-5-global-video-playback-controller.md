...
- [ ] [AI-Review][High] Логика IntersectionObserver не соответствует задуманному поведению: условие `!entry.isIntersecting` срабатывает только при полном исчезновении из viewport (0%), а не при 20% видимости как задумано. Нужно проверять `entry.intersectionRatio < 0.2`. `[src/components/media/VideoPlayer.tsx:67]`
- [ ] [AI-Review][Medium] Необязательная и запутывающая зависимость `videoRef` в `useEffect`: объекты ref не реактивны, их включение в зависимости бессмысленно и является anti-pattern. `[src/components/media/VideoPlayer.tsx:76]`
- [ ] [AI-Review][Medium] Двойной ререндер из-за Derived state при сбросе `hasError`: сброс состояния ошибки во время рендера через `setPrevSrc(src); setHasError(false)` вызывает принудительный синхронный ререндер. Лучше использовать `key={src}` на компоненте. `[src/components/media/VideoPlayer.tsx:47-51]`
- [ ] [AI-Review][Low] Возможная коллизия ID при фоллбэке в `PostCard.tsx`: использование `post.id` в качестве `videoId` может привести к коллизиям, если UUID поста совпадёт с UUID медиафайла. Лучше использовать префикс `fallback-video-${post.id}`. `[src/components/feed/PostCard.tsx:113]`
