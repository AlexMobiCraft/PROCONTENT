...
- Валидация по **оригиналу ДО сжатия**: 0-байт → MIME-whitelist → HEIC-детект → размер ≤ 50 МБ.
...
- `src/features/profile/api/profileApi.ts` -- `uploadAvatar`: новый порядок валидаций (0-byte → MIME → HEIC → 50 МБ), вызов `compressAvatar`, upload с `contentType` из blob.
...
- [ ] `src/features/profile/api/profileApi.ts` -- порядок в `uploadAvatar`: 0-byte → MIME-whitelist → HEIC-детект (`type` включает `heic`/`heif` ИЛИ имя `.heic`/`.heif` → throw HEIC-тост) → `file.size > 50*1024*1024` → throw 50 МБ-тост. Затем `const toUpload = await compressAvatar(file)`; upload `toUpload` с `{ contentType: toUpload.type, cacheControl, upsert:false }`, путь из `toUpload.name`. Удалить старый `MAX_AVATAR_SIZE`.
...
- [ ] `tests/unit/features/profile/profileApi.test.ts` -- заменить «exceeds 256KB» на «>50MB → throw 50 МБ-тост»; «256KB exactly» оставить зелёным (jsdom: ≤256 КБ → skip, ≤50 МБ → upload); добавить HEIC-кейсы (`image/heic` и имя `.heic` → throw); ассерт `upload` с `contentType`.
