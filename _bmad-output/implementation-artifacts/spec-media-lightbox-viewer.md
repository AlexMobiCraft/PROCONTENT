...
- 2026-04-28: Исправлены два mobile-бага: (1) кнопка развёртки одиночного видео в `PostDetail` — `opacity-0 group-hover:opacity-100` заменено на `opacity-100 md:opacity-0 md:group-hover:opacity-100` (на мобильном hover не срабатывает, кнопка была невидима); (2) свайп-навигация в `MediaLightbox` — убран guard `target.closest('video')` из `handlePointerDown`, свайп теперь работает даже при касании области видео. 83/83 тестов зелёные.
