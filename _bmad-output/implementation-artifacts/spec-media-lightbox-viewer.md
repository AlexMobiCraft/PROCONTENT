...
- 2026-04-28: Исправлены мобильные баги: (1) кнопка развёртки одиночного видео в `PostDetail` — `opacity-0 group-hover:opacity-100` → `opacity-100 md:opacity-0 md:group-hover:opacity-100`; (2) свайп в `MediaLightbox` — убран guard `closest('video')` из `handlePointerDown`; (3) `GalleryGrid` video items — заменён `div[role="button"]` на `div.relative` + прозрачная `<button className="absolute inset-0">`: Android/iOS перехватывают touch на `<video controls>` до всплытия, overlay-кнопка гарантирует перехват тапа. 143/143 тестов зелёные.
