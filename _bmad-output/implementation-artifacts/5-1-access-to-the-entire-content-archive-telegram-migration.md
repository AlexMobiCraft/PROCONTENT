...
- [ ] Task 1: Миграция БД — добавить `telegram_message_id` в таблицу `posts` (AC: #5)
  - [ ] 1.1 Создать `supabase/migrations/025_add_telegram_message_id.sql`
  - [ ] 1.2 Поле: `telegram_message_id BIGINT UNIQUE` (nullable для постов созданных вручную)
  - [ ] 1.3 Создать индекс: `idx_posts_telegram_message_id ON posts(telegram_message_id) WHERE telegram_message_id IS NOT NULL`
  - [ ] 1.4 Применить миграцию: `npx supabase db push`

- [ ] Task 2: Создать скрипт `scripts/telegram_migration.ts` (AC: #1, #2, #3, #4, #5, #6, #7, #8)
  - [ ] 2.1 Парсинг аргументов: `--input <path>`, `--dry-run`, `--resume` (resume читает `.migration-state.json`)
  - [ ] 2.2 Валидация ENV vars: `NEXT_PUBLIC_SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY`
  - [ ] 2.3 Парсинг `result.json`: функция `parseTelegramExport(jsonPath)` → массив Message[]
  - [ ] 2.4 Группировка сообщений: функция `groupMessages(messages)` → Post[] (одиночные + медиагруппы)
  - [ ] 2.5 Загрузка медиа: функция `uploadMedia(filePath, fileName)` → Storage URL с Exponential Backoff
  - [ ] 2.6 Вставка поста в БД с проверкой идемпотентности через `telegram_message_id`
  - [ ] 2.7 Cursor: сохранение/восстановление состояния из `.migration-state.json`
  - [ ] 2.8 Сводный отчёт по завершении

- [ ] Task 3: Установить зависимости для скрипта (AC: #1)
  - [ ] 3.1 `npm install --save-dev tsx dotenv` (если не установлены — `npx tsx` может быть достаточно)
  - [ ] 3.2 Проверить: `npx tsx scripts/sync-stripe-subscriptions.ts --dry-run` работает (паттерн уже используется)
