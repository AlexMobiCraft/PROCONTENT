# Prompt for Blind Hunter

You are an adversarial reviewer. You receive ONLY the diff below. You have NO context about the project, spec, or goals. Your job is to find logic errors, security vulnerabilities, performance issues, syntax errors, and anti-patterns strictly within the provided code changes. Do not assume any external behavior or structure.

Review the following diff and output findings as a Markdown list. Each finding must include a one-line title, the identified issue, and the specific lines from the diff that contain the issue.

```diff
diff --git a/src/app/(admin)/posts/[id]/edit/page.tsx b/src/app/(admin)/posts/[id]/edit/page.tsx
index c4a99c03..88c4abaa 100644
...
+        type: data.type,
     ...
     />
diff --git a/src/features/admin/api/posts.ts b/src/features/admin/api/posts.ts
index 97c4c1b9..11f66006 100644
--- a/src/features/admin/api/posts.ts
+++ b/src/features/admin/api/posts.ts
...
-    // 5. Update order_index + is_cover for retained existing items ÔÇö sequential to avoid rate limiting
+    // 5. Update order_index + is_cover for retained existing items ÔÇö single upsert call
...
@@ -215,6 +228,23 @@ export async function updatePost(input: UpdatePostInput): Promise<void> {
...
+    throw new Error(`Objava ni bila najdena: ${error?.message ?? ''}`, { cause: error })
   ...
   return data
diff --git a/src/features/admin/api/uploadMedia.ts b/src/features/admin/api/uploadMedia.ts
index 59aae253..6be16590 100644
--- a/src/features/admin/api/uploadMedia.ts
+++ b/src/features/admin/api/uploadMedia.ts
...
diff --git a/src/features/admin/components/MediaUploader.tsx b/src/features/admin/components/MediaUploader.tsx
index b67c12c9..6903acf5 100644
--- a/src/features/admin/components/MediaUploader.tsx
+++ b/src/features/admin/components/MediaUploader.tsx
...
diff --git a/src/features/admin/components/PostForm.tsx b/src/features/admin/components/PostForm.tsx
index 63b5afaa..fe2666b7 100644
--- a/src/features/admin/components/PostForm.tsx
+++ b/src/features/admin/components/PostForm.tsx
...
+  // Cleanup ObjectURLs on unmount ÔÇö snapshot the ref to avoid double-revoke
   useEffect(() => {
...
@@ -129,13 +132,21 @@ export function PostForm(props: PostFormProps) {
...
+      setMediaError(`Najve─Ź ${MAX_MEDIA_FILES} datotek je dovoljenih`)
+      return
+    }
     setMediaError(null)
...
diff --git a/src/features/admin/types.ts b/src/features/admin/types.ts
index ae1762cf..25c7343e 100644
...
+  content: z.string().max(50000, 'Vsebina je predolga (max 50.000 znakov)').optional(),
   ...
   category: z
     .string()
diff --git a/tests/unit/features/admin/api/posts.test.ts b/tests/unit/features/admin/api/posts.test.ts
index cf68c44f..89b9733a 100644
--- a/tests/unit/features/admin/api/posts.test.ts
+++ b/tests/unit/features/admin/api/posts.test.ts
