...
- [x] [AI-Review][High] Логика IntersectionObserver не соответствует задуманному поведению: условие `!entry.isIntersecting` срабатывает только при полном исчезновении из viewport (0%), а не при 20% видимости как задумано. Нужно проверять `entry.intersectionRatio < 0.2`. `[src/components/media/VideoPlayer.tsx:67]`
- [x] [AI-Review][Medium] Необязательная и запутывающая зависимость `videoRef` в `useEffect`: объекты ref не реактивны, их включение в зависимости бессмысленно и является anti-pattern. `[src/components/media/VideoPlayer.tsx:76]`
- [x] [AI-Review][Medium] Двойной ререндер из-за Derived state при сбросе `hasError`: сброс состояния ошибки во время рендера через `setPrevSrc(src); setHasError(false)` вызывает принудительный синхронный ререндер. Лучше использовать `key={src}` на компоненте. `[src/components/media/VideoPlayer.tsx:47-51]`
- [x] [AI-Review][Low] Возможная коллизия ID при фоллбэке в `PostCard.tsx`: использование `post.id` в качестве `videoId` может привести к коллизиям, если UUID поста совпадёт с UUID медиафайла. Лучше использовать префикс `fallback-video-${post.id}`. `[src/components/feed/PostCard.tsx:113]`

- [x] [AI-Review][Medium][Docs] Completion Notes для `Task 2` устарели: в story по-прежнему указано, что автопауза по viewport реализована в `useVideoController`, хотя фактическая реализация находится в `VideoPlayer.tsx`. Синхронизировать описание с кодом. `[_bmad-output/implementation-artifacts/stories/2-5-global-video-playback-controller.md:516, src/hooks/useVideoController.ts:13-48, src/components/media/VideoPlayer.tsx:59-76]`
- [x] [AI-Review][Medium][Docs] Completion Notes для `Task 5` вводят в заблуждение: указано, что `useVideoController` замокан в `VideoPlayer.test.tsx`, но текущие тесты проверяют Dumb UI `VideoPlayer` напрямую без мока хука. `[_bmad-output/implementation-artifacts/stories/2-5-global-video-playback-controller.md:519, tests/unit/components/media/VideoPlayer.test.tsx:1-281]`
- [x] [AI-Review][Medium][Docs] Completion Notes для `Task 6` устарели: в story сказано, что `GalleryGrid` рендерит `VideoPlayer` и что тест мокает `VideoPlayer`, но фактически используются `VideoPlayerContainer` и его мок. `[_bmad-output/implementation-artifacts/stories/2-5-global-video-playback-controller.md:520, src/components/feed/GalleryGrid.tsx:127-139, tests/unit/components/feed/GalleryGrid.test.tsx:6-29]`
- [x] [AI-Review][Medium][Docs] Dev Agent Record / File List больше не синхронизирован с реализацией: запись для `useVideoController.ts` всё ещё упоминает удалённый `try-catch` вокруг `pause()`, а запись про story-файл содержит устаревший статус `review`. `[_bmad-output/implementation-artifacts/stories/2-5-global-video-playback-controller.md:621,627]`

- [x] [AI-Review][High] Нарушение локализации: заменить захардкоженный русский текст «Ваш браузер не поддерживает видео.» и «Ошибка загрузки видео» на корректные словенские fallback-строки. `[src/components/media/VideoPlayer.tsx:93,119]`
- [x] [AI-Review][Medium] Утечка стейта (State Leak): перенести `key={props.src}` из `VideoPlayerContainer.tsx` на уровень выше или добавить логику сброса `activeVideoId` при изменении `props.src`, так как текущий маунт/анмаунт `VideoPlayer` не триггерит cleanup хука. `[src/features/feed/components/VideoPlayerContainer.tsx:17]`
- [x] [AI-Review][Medium] Нарушение доступности (A11y): добавить `role="alert"` или `aria-live="polite"` к контейнеру с ошибкой для поддержки скринридеров. `[src/components/media/VideoPlayer.tsx:86-95]`
- [x] [AI-Review][Low] Жестко заданный Aspect Ratio: изменить `aspectRatio="16/9"` на динамический `post.type === 'video' ? '16/9' : '4/5'` для поддержки вертикальных видео. `[src/components/feed/PostCard.tsx:117]`

### Review Follow-ups (AI) (Round 14)

- [x] [AI-Review][High] Исправить ID Collision/Logic в `PostCard.tsx`: использовать `post.mediaItem?.id` как приоритетный `videoId` вместо `post.media?.[0]?.id` для одиночных видео, чтобы избежать рассинхрона со store. `[src/components/feed/PostCard.tsx:112-114]`
- [x] [AI-Review][Medium] Исправить предупреждение `react-hooks/exhaustive-deps` в `VideoPlayer.tsx`: добавить `videoRef` в массив зависимостей `useEffect` для `IntersectionObserver`. `[src/components/media/VideoPlayer.tsx:71]`
- [x] [AI-Review][Medium] Убрать дублирование файлов в `File List` внутри стори: `useVideoController.ts` и `VideoPlayerContainer.tsx` не должны быть одновременно в «Новые» и «Изменённые». `[_bmad-output/implementation-artifacts/stories/2-5-global-video-playback-controller.md:635-657]`
- [x] [AI-Review][Low] Оптимизировать A11y в `VideoPlayer.tsx`: оставить либо `title={alt}`, либо `aria-label={alt}` на теге `<video>`, чтобы избежать избыточности для скринридеров. `[src/components/media/VideoPlayer.tsx:107-111]`

- [x] [AI-Review][High][Logic] Исправить `resolveMediaSrc` в `LazyMediaWrapper.tsx`: при отсутствии `thumbnail_url` для видео не возвращать `.mp4` в `next/image`, так как это ломает лоадер. Использовать прозрачный placeholder или пустую строку. `[src/components/media/LazyMediaWrapper.tsx:45-52]`
- [x] [AI-Review][Medium][A11y] Добавить `eslint-disable-next-line` для `jsx-a11y/media-has-caption` в `VideoPlayer.tsx`, так как наличие текста внутри `<video>` снова триггерит это правило. `[src/components/media/VideoPlayer.tsx:102]`
- [x] [AI-Review][Medium][Test] Изолировать `PostCard.test.tsx`: добавить мок для `GalleryGrid`, чтобы превратить тест из интеграционного в честный юнит-тест. `[tests/unit/components/feed/PostCard.test.tsx:26]`
- [x] [AI-Review][Low][Style] Отрефакторить `PostCardSkeleton` в `PostCard.tsx`: заменить интерполяцию классов на использование утилиты `cn()`. `[src/components/feed/PostCard.tsx:257]`

### Review Follow-ups (AI) (Round 16)

- [x] [AI-Review][Medium][A11y] Потеря контекста для скринридеров в `VideoPlayer.tsx`: контейнер ошибки `role="alert"` должен содержать `aria-label={`Napaka pri nalaganju videa: ${alt}`}` для идентификации контента. `[src/components/media/VideoPlayer.tsx:85]`
- [x] [AI-Review][Medium][Logic] Уязвимость ключа ремаунта в `VideoPlayerContainer.tsx`: использовать `key={`${props.videoId}-${props.src}`}` вместо только `src` для гарантированного сброса стейта при смене ID без смены URL. `[src/features/feed/components/VideoPlayerContainer.tsx:31]`
- [x] [AI-Review][Low][Docs] Пропущенный тип `'multi-video'` в `PostCardSkeleton`: обновить сигнатуру пропса `mediaType: 'photo' | 'video' | 'gallery' | 'multi-video'`. `[src/components/feed/PostCard.tsx:236]`
...
-   **Task 2**: `useVideoController(videoId)` — хук управляет `videoRef` и store-логикой: автопауза при смене активного видео (useEffect по `isActive`), cleanup при unmount. `handlePause` защищён от stale closure через `useFeedStore.getState()`. **IntersectionObserver (автопауза по viewport) реализован в `VideoPlayer.tsx`, а не в хуке** (рефакторинг Round 4: VideoPlayer стал Dumb UI, получил собственный ref и onPlay/onPause через VideoPlayerContainer).
-   **Task 3**: `VideoPlayer` — Dumb UI компонент. Принимает `onPlay`, `onPause`, `ref` (от VideoPlayerContainer), `isLoading` (skeleton). Содержит IntersectionObserver для автопаузы по viewport (`threshold: 0.2`, `intersectionRatio < 0.2`). `key={src}` в VideoPlayerContainer обеспечивает сброс `hasError` при смене src (без derived state).
-   **Task 4**: 10 тестов для `useVideoController` — покрыты все ACs #9 (5 минимум).
-   **Task 5**: 28 тестов для `VideoPlayer` — покрыты все ACs #8 (8 минимум). `VideoPlayer` — Dumb UI, тестируется напрямую без мока `useVideoController` (компонент не импортирует хук). IntersectionObserver мокируется через `vi.stubGlobal`.
-   **Task 6**: `GalleryGrid` — видео-элементы (main + carousel) рендерятся через **`VideoPlayerContainer`** (Smart wrapper над Dumb `VideoPlayer`). `GalleryGrid.test.tsx` мокирует `VideoPlayerContainer`.
-   **Task 7**: `tsc --noEmit` 0 ошибок. `npm run test` 590/590 (457 + 133 ранее + 20 новых = 590 total).

**Review Follow-ups Round 15 (2026-03-24, claude-sonnet-4-6):**
-   ✅ Resolved [High][Logic]: `LazyMediaWrapper.tsx` — `resolveMediaSrc` и `LazyMediaWrapperContent` исправлены: для видео без `thumbnail_url` теперь возвращается `''` вместо `.mp4` URL. Пустой `src` немедленно ставит `isError=true`, показывает error-fallback без попытки загрузить видео через `next/image`. Тест обновлён: заменена проверка `img src=mp4` на проверку `media-error-fallback` + отсутствия `<img>`.
-   ✅ Resolved [Medium][A11y]: `VideoPlayer.tsx` — добавлен `{/* eslint-disable-next-line jsx-a11y/media-has-caption */}` перед `<video>` тегом. Fallback-текст «Vaš brskalnik ne podpira videa.» внутри `<video>` больше не триггерит правило линтера.
-   ✅ Resolved [Medium][Test]: `PostCard.test.tsx` — добавлен `vi.mock('@/components/feed/GalleryGrid', ...)` с моками `GalleryGrid` и `GalleryGridSkeleton`. Тест «в смешанной галерее видео не оборачивается в ссылку» обновлён: проверяет `gallery-grid` (мок) вместо несуществующего `video-player` внутри реального GalleryGrid.
-   ✅ Resolved [Low][Style]: `PostCard.tsx` — `PostCardSkeleton`: интерполяция `` `mb-4 ${mediaAspectClass} ...` `` заменена на `cn('mb-4 w-full rounded-lg bg-muted animate-pulse', mediaAspectClass)`.
-   **Валидация**: `npm run typecheck` ✅ 0 ошибок, `npm test` ✅ `639/639`.

**Review Follow-ups Round 14 (2026-03-24, claude-sonnet-4-6):**
-   ✅ Resolved [High][Logic]: `PostCard.tsx` — videoId теперь использует `post.mediaItem?.id ?? post.media?.[0]?.id ?? \`fallback-video-${post.id}\`` для точного совпадения со store при наличии id в mediaItem. `MediaItem` интерфейс в `LazyMediaWrapper.tsx` расширен полем `id?: string`. Добавлен тест: `mediaItem.id` используется как приоритет над `media[0].id`.
-   ✅ Resolved [Medium][Lint]: `VideoPlayer.tsx` — `videoRef` добавлен в deps `useEffect` IntersectionObserver для устранения предупреждения `react-hooks/exhaustive-deps`. Добавление безопасно: `videoRef = externalRef ?? internalRef` — стабильная ссылка (оба useRef-объекты), лишних re-runs не будет.
-   ✅ Resolved [Medium][Docs]: File List синхронизирован — `useVideoController.ts` и `VideoPlayerContainer.tsx` удалены из «Изменённые» (оставлены только в «Новые»); добавлены `LazyMediaWrapper.tsx` и обновлены описания.
-   ✅ Resolved [Low][A11y]: `VideoPlayer.tsx` — убран `title={alt}` с `<video>` (дублировал `aria-label`), оставлен только `aria-label`. Тест `<video> имеет атрибут title` удалён из `VideoPlayer.test.tsx` (избыточен: `aria-label` тест сохранён).
-   **Валидация**: `npm run typecheck` ✅ 0 ошибок, `npm test` ✅ `639/639`.

**Review Follow-ups Round 13 (2026-03-24, claude-sonnet-4-6):**
-   ✅ Resolved [High][L10n]: `VideoPlayer.tsx` — «Ошибка загрузки видео» → «Napaka pri nalaganju videa», «Ваш браузер не поддерживает видео.» → «Vaš brskalnik ne podpira videa.». Тесты обновлены соответственно.
-   ✅ Resolved [Medium][A11y]: `VideoPlayer.tsx` — error container получил `role="alert"` и `aria-live="polite"` для поддержки скринридеров. Тест добавлен: проверяет оба атрибута.
-   ✅ Resolved [Medium][StateLeak]: `VideoPlayerContainer.tsx` — добавлен `useEffect([props.src, props.videoId])`, который сбрасывает `activeVideoId` в null при смене src (если этот плеер был активным). Два теста добавлены: сброс при смене src (активный плеер) и НЕ сброс когда другой плеер активен.
-   ✅ Resolved [Low][AspectRatio]: `PostCard.tsx` — `aspectRatio="16/9"` заменён на `aspectRatio={post.type === 'video' ? '16/9' : '4/5'}`. Тесты добавлены: `type=video → '16/9'`, `type=multi-video → '4/5'`.
-   **Валидация**: `npm run typecheck` ✅ 0 ошибок, `npm test` ✅ `639/639`.

**Review Follow-ups Round 12 (2026-03-24, claude-sonnet-4-6):**
-   ✅ Resolved [High]: `VideoPlayer.tsx` — `!entry.isIntersecting` заменён на явный `entry.intersectionRatio < 0.2` в IntersectionObserver callback. Тесты обновлены: добавлен `intersectionRatio: 0.1` в мок-записи IntersectionObserver.
-   ✅ Resolved [Medium]: `VideoPlayer.tsx` — `videoRef` убран из deps массива `useEffect` (ref-объекты стабильны, их включение в deps — anti-pattern). Deps: `[videoRef, hasError, isLoading]` → `[hasError, isLoading]`.
-   ✅ Resolved [Medium]: Derived state удалён из `VideoPlayer.tsx` (`prevSrc` state + `if (prevSrc !== src)` блок). Сброс `hasError` при смене `src` реализован через `key={props.src}` в `VideoPlayerContainer.tsx`. Тест обновлён: использует wrapper-компонент с `key={src}` для проверки ремаунта.
-   ✅ Resolved [Low]: `PostCard.tsx` — `videoId` фоллбэк изменён с `post.id` на `` `fallback-video-${post.id}` `` для предотвращения коллизий UUID. Тест обновлён: ожидает `'fallback-video-post-vid-1'`.
-   ✅ Resolved [Medium][Docs]: Completion Notes для Task 2 обновлены — IntersectionObserver вынесен в `VideoPlayer.tsx` (Round 4), хук отвечает только за store-логику.
-   ✅ Resolved [Medium][Docs]: Completion Notes для Task 5 обновлены — VideoPlayer Dumb UI тестируется напрямую (28 тестов), без мока `useVideoController` (хук не импортируется).
-   ✅ Resolved [Medium][Docs]: Completion Notes для Task 6 обновлены — GalleryGrid использует `VideoPlayerContainer`, тест мокирует `VideoPlayerContainer`.
-   ✅ Resolved [Medium][Docs]: File List синхронизирован — удалено упоминание `try-catch` в `useVideoController.ts`, добавлен `VideoPlayerContainer.tsx` как изменённый файл.
...
- `src/components/media/LazyMediaWrapper.tsx` — добавлен `id?: string` в `MediaItem`; `resolveMediaSrc` + `LazyMediaWrapperContent`: видео без thumbnail_url → `''` (error-fallback) вместо `.mp4` в next/image
- `src/components/media/VideoPlayer.tsx` — Dumb UI рефакторинг: убран useVideoController, добавлены onPlay/onPause/ref/isLoading пропсы, IntersectionObserver для viewport-autopause, skeleton; Memory Leak fix (hasError в deps); videoRef добавлен в deps (Round 14); убран title={alt} (оставлен aria-label, Round 14); eslint-disable media-has-caption (Round 15)
...
- `src/components/feed/PostCard.tsx` — использует VideoPlayerContainer для одиночного видео; убран aria-hidden с media Link; mixed gallery восстанавливает клики по изображениям; Like button получает `disabled={isPending}`; videoId: приоритет mediaItem?.id (Round 14)
...
- `tests/unit/components/feed/PostCard.test.tsx` — мок обновлён на VideoPlayerContainer; добавлены тесты на mixed gallery navigation и `disabled` для Like; обновлён тест fallback videoId; добавлен тест mediaItem.id priority (Round 14); добавлен мок GalleryGrid, обновлён тест «смешанная галерея видео не в ссылке» (Round 15)
- `tests/unit/components/media/VideoPlayer.test.tsx` — добавлены тесты: IntersectionObserver disconnect on error, onPause on error, переход `isLoading=true -> false`; обновлены тесты IntersectionObserver (intersectionRatio: 0.1); обновлён тест сброса hasError (wrapper с key={src}); удалён тест title атрибута (Round 14)
- `tests/unit/components/media/LazyMediaWrapper.test.tsx` — обновлён тест video без thumbnail_url: ожидает error-fallback вместо img с mp4 src (Round 15)
- `tests/unit/hooks/useVideoController.test.ts` — 10 тестов (исходные 8 + cleanup at unmount)
- `.gitattributes` — нормализация LF для всех исходников
- `_bmad-output/implementation-artifacts/stories/2-5-global-video-playback-controller.md` — story обновлена
...
- **2026-03-24** — Code Review (Amelia, Round 12, YOLO): подтверждены 4 незакрытых runtime-проблемы (`IntersectionObserver`, deps `videoRef`, derived state, fallback `videoId`) и добавлены 4 новых замечания по рассинхронизации story-документации с кодом (`Completion Notes`, `File List`). `npm run typecheck` ✅, targeted `vitest` ✅ `131/131`, story-scoped `eslint` ⚠️ `1 warning` в моке `GalleryGrid.test.tsx`. Статус остаётся `in-progress`.
- **2026-03-24** — Review Follow-ups Round 12: `intersectionRatio < 0.2` (High, IntersectionObserver), убран `videoRef` из deps (Medium), `key={src}` в VideoPlayerContainer вместо derived state (Medium), `fallback-video-${post.id}` prefix (Low), синхронизированы Completion Notes Task 2/5/6 и File List с реальным кодом (4× Medium[Docs]). 634/634 тестов. Все 8 пунктов закрыты. Статус: review.
- **2026-03-24** — Review Follow-ups Round 13: словенская локализация в VideoPlayer (High: «Napaka pri nalaganju videa», «Vaš brskalnik ne podpira videa.»), `role="alert"` + `aria-live="polite"` на error container (Medium, A11y), State Leak fix в VideoPlayerContainer (Medium: useEffect сбрасывает activeVideoId при смене src), динамический aspectRatio в PostCard (Low: video→16/9, multi-video→4/5). 639/639 тестов. Все 4 оставшихся пункта закрыты. Статус: review.
- **2026-03-24** — Review Follow-ups Round 14: videoId приоритет `mediaItem?.id` в PostCard + `id?` в MediaItem (High), `videoRef` в deps IntersectionObserver (Medium, ESLint), File List дублирование устранено (Medium, Docs), убран `title={alt}` из `<video>` (Low, A11y). 639/639 тестов. Все 4 пункта закрыты. Статус: review.
- **2026-03-24** — Review Follow-ups Round 15: `resolveMediaSrc` в LazyMediaWrapper возвращает `''` вместо `.mp4` для видео без thumbnail (High, Logic), eslint-disable media-has-caption в VideoPlayer (Medium, A11y), мок GalleryGrid в PostCard.test.tsx (Medium, Test), `cn()` в PostCardSkeleton (Low, Style). 639/639 тестов. Все 4 пункта закрыты. Статус: review.
- **2026-03-24** — Code Review (Amelia, Round 16): Найдено 3 проблемы (2 Medium, 1 Low). Добавлены Action Items: A11y контекст ошибки, композитный ключ ремаунта, типы в скелетоне. Статус изменён на `in-progress`.
- **2026-03-24** — Review Follow-ups Round 16: `aria-label` с alt-контекстом на error container VideoPlayer (Medium, A11y), `key={videoId-src}` в VideoPlayerContainer (Medium, Logic), `'multi-video'` в PostCardSkeleton union type (Low, Docs). 640/640 тестов. Все 3 пункта закрыты. Статус: review.
- **2026-03-24** — Review Follow-ups Round 17: Удалено скрытие скроллбара в карусели GalleryGrid (A11y/UX), оптимизирована сортировка в GalleryGrid (Perf), добавлен `<p>` в fallback-текст VideoPlayer (HTML/A11y). 640/640 тестов. Все замечания закрыты. Статус: done.
