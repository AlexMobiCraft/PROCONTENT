...
- [ ] [Review][Patch] В story-файле зафиксирован реальный `CRON_SECRET` в SQL-примере ручной настройки pg_cron; значение нужно удалить из документа, заменить placeholder-ом и ротировать секрет в окружении [6-2-pg-cron-automatic-publishing-of-scheduled-posts.md:292-293]
- [ ] [Review][Patch] Инструкция `Не создавай никаких других файлов — вся логика в одном Route Handler` противоречит Tasks / Subtasks и File List, где уже требуются тесты, миграция и env-изменения [6-2-pg-cron-automatic-publishing-of-scheduled-posts.md:97,304-308]
- [ ] [Review][Patch] Subtask 1.5 всё ещё описывает `fetch('/api/notifications/new-post', ...)`, хотя ниже story уже закрепляет вызов через абсолютный URL с `NEXT_PUBLIC_SITE_URL`; формулировки нужно синхронизировать [6-2-pg-cron-automatic-publishing-of-scheduled-posts.md:44,168-175]
- [ ] [Review][Patch] Reference на `_bmad-output/planning-artifacts/product-brief-scheduled-publishing.md` битый: файл отсутствует по указанному пути и сейчас находится только в `planning-artifacts/arhiv/scheduled publishing/` [6-2-pg-cron-automatic-publishing-of-scheduled-posts.md:252]
- [ ] [Review][Patch] `File List` включает `.env.local`, хотя файл gitignored; лучше пометить его как local-only manual step или убрать из списка изменённых repo-файлов [6-2-pg-cron-automatic-publishing-of-scheduled-posts.md:307]
