...
- [x] Task 1: Создать Route Handler `src/app/api/cron/publish/route.ts` (AC: 1, 2, 3, 4, 5, 6)
  ...
  - [x] 1.5 Цикл по опубликованным постам: `fetch('${process.env.NEXT_PUBLIC_SITE_URL}/api/notifications/new-post', ...)` per post в try/catch (изоляция ошибок)
...
- [x] Task 2: Добавить `CRON_SECRET` в конфигурацию окружения (AC: 1)
  ...
  - [x] 2.2 Добавить в Vercel Environment Variables (Production + Preview) — требует ручного добавления в Vercel Dashboard
...
- [x] [Review][Patch] Migration создаёт нерабочую cron-задачу при применении через `supabase db push` — placeholder `YOUR_APP_URL`/`YOUR_CRON_SECRET` не заменены; нет guard-проверки [supabase/migrations/038_pg_cron_publish_scheduled_posts.sql:20-28] — RESOLVED: DO блок с RAISE EXCEPTION при placeholder-значениях
- [x] [Review][Patch] Тестовые моки `mockFetch` не содержат `response.body` — вызов `body?.cancel()` фактически не тестируется (проходит как no-op) [tests/unit/app/api/cron/publish/route.test.ts:73] — RESOLVED: добавлен `body: { cancel: vi.fn() }` во все mock-ответы, HTTP 4xx тест проверяет `mockCancel` был вызван
...
### Review Findings (Round 4) — 2026-04-03

- [x] [Review][Patch] В story-файле зафиксирован реальный `CRON_SECRET` в SQL-примере ручной настройки pg_cron; значение нужно удалить из документа, заменить placeholder-ом и ротировать секрет в окружении [6-2-pg-cron-automatic-publishing-of-scheduled-posts.md:292-293] — RESOLVED: заменены реальный URL и секрет на `YOUR_APP_URL`/`YOUR_CRON_SECRET`; ⚠️ требуется ротация CRON_SECRET в Vercel и Supabase
- [x] [Review][Patch] Инструкция `Не создавай никаких других файлов — вся логика в одном Route Handler` противоречит Tasks / Subtasks и File List, где уже требуются тесты, миграция и env-изменения [6-2-pg-cron-automatic-publishing-of-scheduled-posts.md:97,304-308] — RESOLVED: инструкция уточнена до "основная логика в одном Route Handler"
- [x] [Review][Patch] Subtask 1.5 всё ещё описывает `fetch('/api/notifications/new-post', ...)`, хотя ниже story уже закрепляет вызов через абсолютный URL с `NEXT_PUBLIC_SITE_URL`; формулировки нужно синхронизировать [6-2-pg-cron-automatic-publishing-of-scheduled-posts.md:44,168-175] — RESOLVED: subtask 1.5 обновлён на `fetch('${NEXT_PUBLIC_SITE_URL}/api/notifications/new-post', ...)`
- [x] [Review][Patch] Reference на `_bmad-output/planning-artifacts/product-brief-scheduled-publishing.md` битый: файл отсутствует по указанному пути и сейчас находится только в `planning-artifacts/arhiv/scheduled publishing/` [6-2-pg-cron-automatic-publishing-of-scheduled-posts.md:252] — RESOLVED: путь исправлен на `_bmad-output/planning-artifacts/arhiv/scheduled publishing/`
- [x] [Review][Patch] `File List` включает `.env.local`, хотя файл gitignored; лучше пометить его как local-only manual step или убрать из списка изменённых repo-файлов [6-2-pg-cron-automatic-publishing-of-scheduled-posts.md:307] — RESOLVED: `.env.local` помечен как `(local-only, gitignored — ручной шаг)`
...
Основная логика сосредоточена в одном Route Handler. Дополнительно требуются: тесты (`tests/unit/app/api/cron/publish/route.test.ts`), миграция (`supabase/migrations/038_...`) и env-изменения (`.env.local`, `.env.example`).
...
- [Source: _bmad-output/planning-artifacts/arhiv/scheduled publishing/product-brief-scheduled-publishing.md] — бизнес-контекст
...
- ✅ Resolved review finding [Patch] Round 3: миграция 038 переписана в DO блок с RAISE EXCEPTION если `v_url = 'YOUR_APP_URL'` или `v_secret = 'YOUR_CRON_SECRET'` — `supabase db push` теперь упадёт с явной ошибкой вместо создания нерабочей cron-задачи
- ✅ Resolved review finding [Patch] Round 3: все `mockFetch.mockResolvedValue*` обновлены с `body: { cancel: vi.fn() }`; HTTP 4xx тест явно проверяет `expect(mockCancel).toHaveBeenCalledTimes(1)`
- ✅ Resolved review finding [Patch] Round 4: реальный CRON_SECRET удалён из story-файла, заменён на placeholder — ⚠️ требуется ротация секрета в Vercel и Supabase
- ✅ Resolved review finding [Patch] Round 4: инструкция Dev Notes уточнена — убрано противоречие с File List и Tasks
- ✅ Resolved review finding [Patch] Round 4: subtask 1.5 синхронизирован — указан абсолютный URL через NEXT_PUBLIC_SITE_URL
- ✅ Resolved review finding [Patch] Round 4: Reference исправлен на корректный путь `arhiv/scheduled publishing/`
- ✅ Resolved review finding [Patch] Round 4: `.env.local` в File List помечен как `local-only, gitignored`

### Ручные шаги для Task 3 (Supabase pg_cron)

**Шаг 1:** В Supabase Dashboard → Settings → Extensions → убедиться что `pg_net` включён.

**Шаг 2:** В Supabase SQL Editor выполнить (заменить `YOUR_APP_URL` и `YOUR_CRON_SECRET` на реальные значения из окружения):
...
- `.env.local` (local-only, gitignored — ручной шаг: добавить CRON_SECRET, NOTIFICATION_API_SECRET)
...
- 2026-04-02: Addressed code review Round 3 findings — 2 patch items resolved (migration guard-check DO block, mockFetch body.cancel testing). 14 тестов — все проходят.
- 2026-04-03: Addressed code review Round 4 findings — 5 patch items resolved (все изменения только в story-документе: удалён реальный CRON_SECRET, исправлен Dev Notes, синхронизирован subtask 1.5, исправлен Reference, помечен .env.local).
