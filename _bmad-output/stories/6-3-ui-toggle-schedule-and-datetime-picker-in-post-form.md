...
- [x] Task 1: Расширить Zod-схему и типы (AC: 1, 2, 3)
  - [x] 1.1 Добавить `status` и `scheduled_at` в `PostFormSchema` (`src/features/admin/types.ts`)
  - [x] 1.2 `status`: `z.enum(['draft', 'scheduled', 'published']).default('published')` — НЕ optional, дефолт `published`
  - [x] 1.3 `scheduled_at`: `z.string().nullable().optional()` — ISO datetime string
  - [x] 1.4 Добавить кросс-валидацию через `.superRefine()`: если `status === 'scheduled'` → `scheduled_at` обязательно + в будущем

- [x] Task 2: Обновить `createPost()` и `updatePost()` в API (AC: 4, 5, 6, 7, 8)
  - [x] 2.1 `createPost()` — принять `status` и `scheduled_at` из формы
  - [x] 2.2 При `status='scheduled'`: INSERT с `is_published: false`, `status: 'scheduled'`, `scheduled_at`, `published_at: null`
  - [x] 2.3 При `status='published'` (immediate): текущее поведение без изменений (`is_published: true`, `published_at` после медиа)
  - [x] 2.4 `updatePost()` — поддержать обновление `status` и `scheduled_at`
  - [x] 2.5 При смене `scheduled → published` (немедленная публикация): `is_published=true`, `published_at=now()`, `scheduled_at=null`; запустить email-уведомление через `POST /api/notifications/new-post`
  - [x] 2.6 При смене `published → scheduled` (перепланирование уже опубликованного): **ЗАПРЕТИТЬ** — пост с `published_at IS NOT NULL` нельзя вернуть в `scheduled`
  - [x] 2.7 При смене `scheduled → draft` (отмена): `is_published=false`, `scheduled_at=null`, `published_at=null`

- [x] Task 3: Добавить UI toggle + datetime picker в PostForm (AC: 1, 2, 3, 6, 7, 8)
  - [x] 3.1 Создать секцию «Режим публикации» в PostForm между секцией curation toggles и кнопкой submit
  - [x] 3.2 Toggle: два `<button>` с `aria-pressed`, визуально объединённые в segmented control (не checkbox/switch). Текст кнопок: `font-sans text-xs font-medium tracking-[0.2em] uppercase` (editorial style per UX spec)
  - [x] 3.3 Активное состояние toggle: `bg-primary text-primary-foreground`. Неактивное: `bg-transparent hover:bg-primary/10`. Focus: `focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:outline-none`. Transition: `transition-colors` (UX-DR9)
  - [x] 3.4 Conditional rendering: `<input type="datetime-local">` при `status === 'scheduled'`
  - [x] 3.5 Preview текст: `text-xs text-muted-foreground tracking-[0.1em]` через `Intl.DateTimeFormat('sl-SI', { dateStyle: 'long', timeStyle: 'short', timeZone: ... })` + timezone abbr
  - [x] 3.6 Inline error: `errors.scheduled_at && <p className="text-destructive text-sm" role="alert">{message}</p>`. При ошибке datetime input получает `border-destructive aria-invalid="true"`
  - [x] 3.7 При edit mode — предзаполнить toggle и datetime из `initialData.status` и `initialData.scheduled_at`
  - [x] 3.8 Текст кнопки submit: «Objavi» при immediate, «Načrtuj» при scheduled. Стиль кнопки — текущий editorial outline (`border border-primary font-sans text-xs font-medium tracking-[0.2em] uppercase`), меняется только текст

- [x] Task 4: Написать тесты (AC: 1–8)
  - [x] 4.1 PostForm: toggle видим, переключается, datetime picker появляется/скрывается
  - [x] 4.2 PostForm: inline error при пустом datetime в режиме scheduled
  - [x] 4.3 PostForm: submit вызывает createPost со `status='scheduled'` и `scheduled_at`
  - [x] 4.4 PostForm: edit mode pre-fills toggle и datetime для scheduled поста
  - [x] 4.5 API: createPost с `status='scheduled'` → INSERT с `is_published=false`
  - [x] 4.6 API: updatePost `scheduled → published` → `is_published=true`, email notification triggered
  - [x] 4.7 API: updatePost `scheduled → draft` → `scheduled_at=null`
  - [x] 4.8 Zod: валидация отклоняет `status='scheduled'` без `scheduled_at`
  - [x] 4.9 Zod: валидация отклоняет `scheduled_at` в прошлом при `status='scheduled'`
...
- ✅ Task 1: Расширена Zod-схема `PostFormSchema` — добавлены `status` (enum с default 'published') и `scheduled_at` (nullable string) + кросс-валидация через `superRefine()` (8 новых тестов)
- ✅ Task 2: Обновлены `createPost()` и `updatePost()` — поддержка `scheduled`, `draft`, `published` статусов с корректными `is_published`/`published_at`/`scheduled_at` полями. Создан Route Handler `POST /api/posts/publish` для немедленной публикации scheduled поста с email-уведомлением (10 тестов)
- ✅ Task 3: Добавлена секция scheduling в PostForm — segmented control toggle (Objavi zdaj / Načrtuj objavo), нативный `<input type="datetime-local">`, preview через `Intl.DateTimeFormat('sl-SI')`, inline validation error. Динамический текст submit кнопки
- ✅ Task 4: Полное тестовое покрытие — 7 PostForm scheduling тестов, 3 API scheduling теста, 8 Zod validation тестов, 10 publish route handler тестов
- ℹ️ Исправлен pre-existing broken тест `shows media required error when no media items on submit` → `allows submit without media items (creates text post)` — форма никогда не блокировала отправку без медиа
- ✅ Code Review Fix 1: `/api/posts/publish` auth переключён с NOTIFICATION_API_SECRET на Supabase user session (cookies) — route handler теперь вызывается с клиента
- ✅ Code Review Fix 2: PostForm.onSubmit вызывает `/api/posts/publish` при переходе scheduled→published (AC8) — email-уведомление корректно отправляется
- ✅ Code Review Fix 3: Guard published→scheduled — disabled кнопка "Načrtuj objavo" для уже опубликованных постов + guard в updatePost (AC 2.6)

### Change Log

- 2026-04-03: Story 6.3 реализована — scheduling toggle, datetime picker, API scheduling, publish route handler
- 2026-04-03: Code Review исправления — publish route auth (user session), PostForm подключён к publish endpoint, guard published→scheduled

### File List

**Изменённые:**
- `src/features/admin/types.ts` — добавлены `status`, `scheduled_at` в PostFormSchema + superRefine
- `src/features/admin/components/PostForm.tsx` — scheduling section (toggle + datetime + preview + error), dynamic submit text, вызов publish endpoint при scheduled→published, disabled toggle для published постов
- `src/features/admin/api/posts.ts` — createPost/updatePost с scheduling support, guard published→scheduled
- `tests/unit/features/admin/types.test.ts` — 8 новых тестов scheduling validation
- `tests/unit/features/admin/api/posts.test.ts` — 4 новых теста scheduling API (включая guard published→scheduled)
- `tests/unit/features/admin/components/PostForm.test.tsx` — 9 новых тестов scheduling UI (включая disabled toggle, publish endpoint call)

**Новые:**
- `src/app/api/posts/publish/route.ts` — Route Handler для немедленной публикации scheduled поста (auth: Supabase user session)
- `tests/unit/app/api/posts/publish/route.test.ts` — 9 тестов publish endpoint (user session auth)
