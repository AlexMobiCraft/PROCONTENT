...
- 53 patch findings (8 CRITICAL, 16 HIGH, 29 MEDIUM) left as action items for resolution
...
- [ ] [Review][Patch] FK ON UPDATE CASCADE enables silent data corruption — `supabase/migrations/021_create_categories.sql:59-62`
...
- [ ] [Review][Patch] Missing UUID format validation in TypeScript types — `src/types/supabase.ts:42-45`
...
- [ ] [Review][Patch] Missing `anon` RLS policy — `supabase/migrations/021_create_categories.sql:18-21`
...
- [ ] [Review][Patch] Race condition in migration: FK constraint applied without pre-validation — `supabase/migrations/021_create_categories.sql:54-62`
...
- [ ] [Review][Patch] Race condition on category deletion — concurrent post creation could orphan data — `src/features/admin/api/categories.ts:36-47`
...
- [ ] [Review][Patch] ON DELETE RESTRICT without app-level enforcement — `src/features/admin/components/CategoryManager.tsx`
...
- [ ] [Review][Patch] RLS policy not reflected in TypeScript type; no error feedback — `src/types/supabase.ts:50-55` + `src/features/admin/components/CategoryManager.tsx`
...
- [ ] [Review][Patch] NULL handling incomplete — NULL rows not backfilled, nullable FK ambiguous — `supabase/migrations/021_create_categories.sql:51-55` + `src/features/admin/components/PostForm.tsx:243-244`
...
- [ ] [Review][Patch] Slug collision risk and unclear error message — `src/features/admin/api/categories.ts:20-34`
...
- [ ] [Review][Patch] No server-side validation against live categories before form submit — `src/features/admin/components/PostForm.tsx:240-250`
...
- [ ] [Review][Patch] Migration missing pre-check for posts.category column existence — `supabase/migrations/021_create_categories.sql`
...
- [ ] [Review][Patch] createCategory: null data return despite success on .single() — `src/features/admin/api/categories.ts:33`
...
- [ ] [Review][Patch] deleteCategory: silent success — no verification that deletion occurred — `src/features/admin/api/categories.ts:36-47`
...
- [ ] [Review][Patch] updatePost: snapshot rollback loses concurrent updates — `src/features/admin/api/posts.ts:150-260`
...
- [ ] [Review][Patch] Rollback failures silent — no AggregateError thrown — `src/features/admin/api/posts.ts:241-265`
...
- [ ] [Review][Patch] Post deleted during snapshot-to-update window — no affected_rows validation — `src/features/admin/api/posts.ts:163-176`
...
- [ ] [Review][Patch] MAX_MEDIA_FILES no server-side validation — `src/features/admin/api/posts.ts:43-45` + `138-140`
...
- [ ] [Review][Patch] DRY violation: getCategories duplicated across client/server — `src/features/admin/api/categories.ts:11-17` vs `categoriesServer.ts:5-11`
...
- [ ] [Review][Patch] Race condition: DB delete successful, Storage cleanup fails → orphaned files — `src/features/admin/api/posts.ts:225-240`
...
- [ ] [Review][Patch] slug collision: no normalization, error message unclear — `src/features/admin/api/categories.ts:20-34`
...
- [ ] [Review][Patch] Server-side re-validation missing: category deleted after form load, before submit — `src/features/admin/api/posts.ts:60-80`
...
- [ ] [Review][Patch] Error code hardcoded (23503) — incomplete fallback for FK violations — `src/features/admin/api/categories.ts:42-45`
...
- [ ] [Review][Patch] XSS in aria-label — post.category not sanitized — `src/components/feed/PostCard.tsx:118`
...
- [ ] [Review][Patch] toSlug produces empty string on special chars — `src/features/admin/components/CategoryManager.tsx:14-22`
...
- [ ] [Review][Patch] Race condition in handleDelete — rapid double-click deletes twice — `src/features/admin/components/CategoryManager.tsx:69-79`
...
- [ ] [Review][Patch] PostForm setValue double-call or skip — categories change trigger useEffect twice — `src/features/admin/components/PostForm.tsx:132-137`
...
- [ ] [Review][Patch] PostCard keyboard UX broken — Space/Enter key still navigates — `src/components/feed/PostCard.tsx:107-126`
...
- [ ] [Review][Patch] Error detection string mismatch — checks for "već obstaja", API returns "je obstaja" — `src/features/admin/components/CategoryManager.tsx:57`
...
- [ ] [Review][Patch] Disabled state not propagated to Input — isAdding=true disables button only — `src/features/admin/components/CategoryManager.tsx:102-104`
...
- [ ] [Review][Patch] Missing null check for post.category — undefined rendered in aria-label/text — `src/components/feed/PostCard.tsx:110-126`
...
- [ ] [Review][Patch] Touch target sizes too small — icon-sm button may violate 44px minimum — `src/features/admin/components/CategoryManager.tsx:139-150`
...
- [ ] [Review][Patch] Error code detection incomplete fallback — no message fallback if error.code missing — `src/features/admin/components/CategoryManager.tsx:51-65`
...
- [ ] [Review][Patch] Missing error.tsx for categories page — errors bubble to global handler — `src/app/(admin)/categories/page.tsx`
...
- [ ] [Review][Patch] FeedContainer pending likes leakage — stale state when switching category — `src/features/feed/components/FeedContainer.tsx:470-473`
...
- [ ] [Review][Patch] categories.test: .order() assertion not verified in chain — `tests/unit/features/admin/api/categories.test.ts:50`
...
- [ ] [Review][Patch] categories.test: .single() not verified in mock chain — `tests/unit/features/admin/api/categories.test.ts:26`
...
- [ ] [Review][Patch] PostForm: setValue may skip if categories loading delayed — `tests/unit/features/admin/components/PostForm.test.tsx:132-137`
...
- [ ] [Review][Patch] posts.test: uploadedUrls partial batch failure not tested — `tests/unit/features/admin/api/posts.test.ts:74-79`
...
- [ ] [Review][Patch] CategoryManager/PostCard: race condition on rapid double-click — `src/features/admin/components/CategoryManager.tsx:32` + `src/components/feed/PostCard.tsx:64`
...
- [ ] [Review][Patch] PostCard: stopPropagation incomplete — event.stopPropagation() not verified — `tests/unit/components/feed/PostCard.test.tsx:713-727`
...
- [ ] [Review][Patch] PostCard category pill: keyboard activation test missing — `tests/unit/components/feed/PostCard.test.tsx`
...
- [ ] [Review][Patch] PostForm: submit before categories loaded race condition — `tests/unit/features/admin/components/PostForm.test.tsx:121-141`
...
- [ ] [Review][Patch] posts.test: snapshot-to-update sequence not verified — `tests/unit/features/admin/api/posts.test.ts:239-265`
...
- [ ] [Review][Patch] PostCard: onLikeToggle without isPending allows race — `tests/unit/components/feed/PostCard.test.tsx:64-67`
...
- [ ] [Review][Patch] PostCard category pill: XSS via aria-label untested — `src/components/feed/PostCard.tsx:118` + test gap
...
- [ ] [Review][Patch] posts.test: makeChain upsert mock missing .select() — `tests/unit/features/admin/api/posts.test.ts:9-19`
...
- [ ] [Review][Patch] toSlug empty string edge case — `tests/unit/features/admin/components/CategoryManager.test.tsx`
...
- [ ] [Review][Patch] null/undefined post.category not handled — `tests/unit/components/feed/PostCard.test.tsx`
...
- [ ] [Review][Patch] MAX_MEDIA_FILES boundary: exactly 10 should pass — `tests/unit/features/admin/api/posts.test.ts:108-114`
...
- [ ] [Review][Patch] Edit mode: stale category test missing — `tests/unit/features/admin/components/PostForm.test.tsx`
...
- [ ] [Review][Patch] Server-side category re-validation test — `tests/unit/features/admin/api/posts.test.ts`
...
- [ ] [Review][Patch] Rollback error propagation test missing — `tests/unit/features/admin/api/posts.test.ts:239-265`
...
- [ ] [Review][Patch] State cleanup between tests — `tests/unit/features/admin/components/CategoryManager.test.tsx:30`
...
- [ ] [Review][Patch] Mock chain verify order enforcement — `tests/unit/features/admin/api/categories.test.ts`
