...
**Execution — инфраструктура (корень B, вне кода) — ✅ ВЫПОЛНЕНО пользователем 2026-06-11:**
- [x] Боевая БД (`/pg/query` или SSH): `CREATE EXTENSION IF NOT EXISTS pg_cron;`. Подтверждено из прода: `pg_extension` → `pg_cron` v1.6, `pg_net` v0.14.0.
- [x] Применить миграцию `038` с `v_url := 'https://www.procontent.si'` (канонический www, НЕ апекс). Подтверждено: `cron.job` → `publish-scheduled-posts`, `active=true`, `schedule='*/5 * * * *'`, `url='https://www.procontent.si/api/cron/publish'`.
- [x] Проверить: `cron.job_run_details` показывает успешные тики каждые 5 мин; `net._http_response` фиксирует ответы Vercel. Корень B закрыт.
...
- **2026-06-11/12 — прод-диагностика и устранение (прямые SELECT через `/pg/query` + HTTP-пробы прод-эндпоинта).** (1) **Корень B закрыт пользователем:** `pg_cron` v1.6 + `pg_net` v0.14.0 установлены, джоб `publish-scheduled-posts` активен (`*/5`, канонический www-URL), `cron.job_run_details` — успешные тики. Исходный диагноз спеки «`pg_cron` не установлен» более не актуален. (2) **Реальный блокирующий корень (корень C) — устаревший `SUPABASE_SERVICE_ROLE_KEY` в Vercel после миграции Cloud → Hetzner.** `net._http_response` показывал на каждый тик `500 {"error":"Server misconfiguration"}` — запрос доходит до функции (не редирект/middleware). Дискриминация через HTTP-пробы: запрос **без** Authorization → `401 Unauthorized` (значит `CRON_SECRET` в рантайме **есть** — guard стр. 42 пройден; первоначальная гипотеза про `CRON_SECRET` опровергнута); запрос **с валидным** Bearer → `500 Server misconfiguration` (значит падает `createAdminClient()` на стр. 56). `NEXT_PUBLIC_SUPABASE_URL` валиден (server-side чтения ленты работают). Ранний ручной тик 14:52 дал `Database error` (ключ был непустой, но старый Cloud → Hetzner Kong отверг) → затем ключ обнулили/не обновили → `Server misconfiguration`. Эталонное значение `SERVICE_ROLE_KEY` идентично в `.env.local` и `hetzner-deploy/.env.server.backup`, принято боевым Kong. **Фикс — infra, вне кода:** в Vercel Production задать `SUPABASE_SERVICE_ROLE_KEY` = Hetzner-ключ (имя в приложении с префиксом `SUPABASE_`, на сервере — `SERVICE_ROLE_KEY`; значение одно) + Redeploy. **Подтверждено:** фикс доехал ~03:47–03:50 UTC 12 июня — тики `net._http_response` сменились `500` → `200 {"published":0,"emailErrors":[]}`, прямая проба эндпоинта тоже `200`. Корень C закрыт.
