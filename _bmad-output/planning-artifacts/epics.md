...
**When** пользователь вводит email и запрашивает код/ссылку
**Then** на почту приходит письмо с OTP или Magic Link
**And** при вводе валидного кода/переходе по ссылке выдается session token (JWT) и происходит редирект на защищенную страницу
**And** при вводе неверного или просроченного кода отображается понятная ошибка интерфейса
**And** инкрементально создается только таблица `profiles` для хранения данных участниц (без предварительного создания таблиц постов или подписок)

### Story 1.3: Публичный лендинг (Landing Page UI) — `in-progress`
...
**Pending:**
- Исправить цену в `PricingSection`: €29 → **€12,99/мес** и **€34/3 мес**
- Вынести `PreviewPostCard` в отдельный файл `src/components/landing/PreviewPostCard.tsx`
- Написать unit-тесты (HeroSection, PreviewPostCard, TestimonialsSection)
...
### Story 1.6: Личный кабинет и управление подпиской
...
### Story 1.7: Onboarding-страница для новых участниц
