...
- Content Editor System (Tiptap + react-markdown)
...
- **Content Editor → Storage:** Инлайн-изображения загружаются в отдельный bucket `inline-images` при редактировании поста, URL вставляется в markdown-контент.
...
### Content Editor System (WYSIWYG + Markdown)

- **Decision:** Tiptap (headless, React) + react-markdown для рендеринга
- **Version:** Tiptap v2.x (React), react-markdown v9.x
- **Rationale:** FR16.2 и FR19.1 требуют легковесный WYSIWYG-редактор для создания постов с инлайн-изображениями. Tiptap — headless-редактор без навязанного UI, идеально кастомизируется под наш дизайн. Поддерживает кастомные расширения для загрузки изображений напрямую в Supabase Storage. react-markdown обеспечивает безопасный рендеринг markdown на клиенте с поддержкой lazy loading (NFR4.2).
- **Affects:** Admin Post Form, Post Detail View, Storage Upload Pipeline

**Ключевые архитектурные паттерны редактора:**

1. **Upload Flow:** Drag & drop / paste изображения → загрузка в Supabase Storage bucket `inline-images` → получение public URL → вставка в markdown как `![alt](url)`.
2. **Render Flow:** `react-markdown` с плагином `remark-gfm` + кастомный компонент `<img>` с `loading="lazy"` (NFR4.2).
3. **Sanitization:** DOMPurify для очистки HTML при рендеринге (XSS-защита).
4. **Режимы:** WYSIWYG-редактор только в админке; на клиенте — рендеринг через react-markdown.
...
- **Inline Images:** Drag & drop / paste в Tiptap → загрузка в bucket `inline-images` → получение public URL → автоматическая вставка `![alt](url)` в markdown-контент → сохранение в колонку `content` таблицы `posts`.
...
- Использовать `loading="lazy"` для всех `<img>` в react-markdown рендерере (NFR4.2).
...
│   │   │   └── lib/            # Upload helpers, markdown serializers
...
│   │   ├── markdown.ts         # react-markdown конфиг, sanitization (DOMPurify)
...
- **Lazy Loading (NFR4.2):** react-markdown с кастомным `<img loading="lazy">` компонентом.
...
- [x] Content Editor System documented (Tiptap + react-markdown)
...
# Установка зависимостей редактора: npm install @tiptap/react @tiptap/starter-kit @tiptap/extension-image react-markdown remark-gfm dompurify
