...
- **PricingSection — recurring baseline:** Две карточки тарифов: **€12,99/месяц** и **€34/3 месяца** (≈€11,33/мес, экономия €4,97). Чек-лист (6 пунктов) общий для обоих планов. CTA ведёт к recurring checkout. Это единственное pricing-состояние вне temporary window и после cutoff `2026-12-01 00:00 Europe/Ljubljana`.
- **PricingSection — temporary offer mode:** В окне `[2026-09-01 00:00, 2026-12-01 00:00)` в `Europe/Ljubljana` server-derived mode заменяет recurring baseline одной карточкой: `€29,00 / 3 mesece`, семантически зачёркнутая `€34,00`, текст `Dostop do obstoječe baze znanja za 3 mesece.` и CTA с доступным именем `Pridruži se zdaj`. Не показываются selector, `MESEČNO`, `€12,99`, `/ mesec`, `≈`, `Prihranek`, copy про cancellation/autorenewal или обещания новых материалов. CTA блокирует повторное нажатие до ответа server redirect gate; при network/redirect failure показывает понятную словенскую ошибку без автоматического retry. Focus должен быть видимым, touch target — не менее 44×44 px, состояние не передаётся только цветом; на 375px, 768px и ≥1024px нет горизонтального переполнения. Exact live copy и Stripe configuration требуют approval gate до production enablement.
